...
class Save(abc.ABC):
    ...
    def finish_up(self):
        ...
        if self.media.input_temp:
            rm(self.media.input)
        if self.output_temp:
            self.clouds.transfer('upload_directory',
                                 os.path.dirname(self.output_))
            if self.output:
                shutil.move(os.path.dirname(self.output_),
                            os.path.dirname(str(self.output)))
            else:
                shutil.rmtree(os.path.dirname(
                    str(self.output_)), ignore_errors=True)
        ...
        def method(*args, **kwargs):
            if name in ['save', 'package']:
                ...
                raise AttributeError(
                    "The object has no attribute {}".format(name))
    ...
    def output(self, output: str = None, clouds: CloudManager = None,
        ...
        if output is None and clouds is None:
            ...
            if clouds.filename is None:
                clouds.filename = os.path.basename(
                    output if output is not None else self.media.input)
            self.output_ = os.path.join(tempfile.mkdtemp(
                prefix='ffmpeg_streaming2_'), clouds.filename)
        else:
...
class Streaming(Save, abc.ABC):
    ...
    def auto_generate_representations(self, heights=None, bitrate=None, ffprobe_bin='ffprobe', include_original=True,
        ...
        self.reps = AutoRep(probe.video_size, probe.bitrate,
                            self.format, heights, bitrate, include_original)
    ...
    def add_filter(self, *_filter: str):
        ...
        @TODO: add documentation
        """
        _filters = self.options.pop('vf', None)
        _filters = (
        ...
        self.options.update({'vf': _filters})

    def watermarking(self, content, font_size=20, interval=5, font_file=None):
        """
        @TODO: add documentation
        """
        filter_value = f'''"drawtext=fontfile={font_file}:fontsize={font_size}:fontcolor=white@1:text='{content}':x=if(eq(mod(t\,5)\,0)\,rand(0\,(W-tw))\,x):y=if(eq(mod(t\,5)\,0)\,rand(0\,(H-th))\,y)"'''
        self.add_filter(filter_value)
