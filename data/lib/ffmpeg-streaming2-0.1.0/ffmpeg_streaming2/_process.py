...
from ffmpeg_streaming2._hls_helper import HLSKeyInfoFile
from ffmpeg_streaming2._utiles import get_time, time_left
...
class Process(object):
    ...
    def _monitor(self):
        ...
        while True:
            ...
            if callable(self.monitor):
                ...
                self.monitor(line, duration, _time, time_left(
                    start_time, _time, duration), self.process)
    ...
    def _thread_mon(self):
        ...
        if thread.is_alive():
            ...
            thread.join()
            error = 'Timeout! exceeded the timeout of {} seconds.'.format(
                str(self.timeout))
            logging.error(error)
