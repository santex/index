...
class S3(Clouds):
    ...
    def upload_directory(self, directory, **options):
        ...
        try:
            for file in files:
                self.s3.upload_file(join(directory, file), bucket_name, join(
                    folder, file).replace("\\", "/"))
        ...
        logging.info(
            "The {} directory was uploaded to Amazon S3 successfully".format(directory))
...
class LINODE(Clouds):
    def __init__(self, **options):
        ...
        self.linode = boto3.client('linode', **options)
        self.err = ClientError
    ...
    def upload_directory(self, directory, **options):
        ...
        try:
            for file in files:
                self.linode.upload_file(join(directory, file), bucket_name, join(
                    folder, file).replace("\\", "/"))
        except self.err as e:
            logging.error(e)
            raise RuntimeError(e)
        ...
        logging.info(
            "The {} directory was uploaded to LINODE successfully".format(directory))
    ...
    def download(self, filename=None, **options):
        ...
        try:
            with filename as f:
                self.linode.download_fileobj(bucket_name, key, f)
            logging.info(f'The {filename.name} file was downloaded')
        except self.err as e:
            logging.error(e)
            raise RuntimeError(e)
...
class GCS(Clouds):
    ...
    def upload_directory(self, directory, **options):
        ...
        for file in files:
            blob = bucket.blob(
                join(folder, file).replace("\\", "/"), **options)
...
class MAS(Clouds):
        ...
        try:
            for file in files:
                self.block_blob_service.create_blob_from_path(
                    container, file, join(directory, file))
        except:
    ...
    def download(self, filename=None, **options):
        ...
        if container is None or blob is None:
            raise ValueError(
                'You should pass a container name and a blob name')
