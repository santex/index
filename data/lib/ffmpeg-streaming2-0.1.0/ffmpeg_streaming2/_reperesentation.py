...
def reduce_bitrate(bitrate: Bitrate, divide: int) -> Bitrate:
    ...
    video = min_bitrate(int(bitrate.video_ / divide)
                        ) if bitrate.video_ is not None and bitrate.video_ != 0 else None
    audio = min_bitrate(int(bitrate.audio_ / divide)
                        ) if bitrate.audio_ is not None and bitrate.audio_ != 0 else None
...
class AutoRep(object):
    def __init__(self, original_size: Size, original_bitrate: Bitrate, _format: Format,
        ...
        if heights is not None and bitrate is not None and len(heights) != len(bitrate):
            raise ValueError(
                "The length of heights list must the same as length of bitrate")
    ...
    def __iter__(self):
        ...
        if not self.is_default:
            height = self.original_size.ratio.calculate_height(
                self.original_size.width, self._format.multiply())
            original = [height] if self.include_original else []
            self.heights = original + \
                list(filter(lambda x: x < height, self.heights))
    ...
    def __next__(self):
        ...
        width = self.original_size.ratio.calculate_width(
            height, self._format.multiply())
        self.index += 1
