...
from ffmpeg_streaming2 import FFProbe, Representation, Size, Bitrate
from ffmpeg_streaming2._media import HLS, DASH
from ffmpeg_streaming2.ffprobe import Streams


class TestStreaming(unittest.TestCase):

    def setUp(self):
        self.src_dir = os.path.join(os.path.dirname(
    ...
    def test_encrypted_hls(self):
        ...
        hls_obj.output(os.path.join(
            self.src_dir, 'encrypted_hls', 'test.m3u8'), stderr=False)
        with open(os.path.join(self.src_dir, 'fixture_test.m3u8')) as test_m3u8:
            expected_m3u8 = test_m3u8.read()
    ...
    def test_dash(self):
        ...
        dash_obj.output(os.path.join(
            self.src_dir, 'dash', 'test.mpd'), stderr=False)
