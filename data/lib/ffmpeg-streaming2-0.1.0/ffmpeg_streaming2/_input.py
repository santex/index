...
"""
from ffmpeg_streaming2._media import Media
from ffmpeg_streaming2._utiles import get_os, cnv_options_to_args
from ffmpeg_streaming2._clouds import Clouds
...
class Capture(object):
    ...
    def _linux(self):
        cap = 'x11grab' if (is_screen := self.options.pop(
            'screen', False)) else 'v4l2'
        return {
            'f': cap,
            'i': self.video
        }
...
def get_from_cloud(_cloud: Clouds, options: dict):
    ...
    if cloud is None:
        ...
        cloud = {'i': _cloud.download(
            save_to, **options), 'is_tmp': save_to is None}
