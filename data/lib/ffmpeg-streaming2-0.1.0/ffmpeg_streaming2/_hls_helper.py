...
from ffmpeg_streaming2._utiles import mkdir, get_path_info
...
class HLSMasterPlaylist:
    ...
    def generate(cls, media, path=None):
        if path is None:
            path = "{}.m3u8".format(os.path.join(
                *get_path_info(media.output_)))
    ...
    def _content(self) -> str:
        ...
        @TODO: add documentation
        """
        content = ['#EXTM3U'] + self._get_version() + \
            self.media.options.get('description', [])

        for rep in self.media.reps:
            if sub_exists := os.path.isfile(
                os.path.dirname(self.media.output_) +
                '/' + self.sub_path(rep)[0]
            ):
                content += sub_info(rep, self.sub_path(rep)[0]) + stream_info(
                    rep, sub_exists) + self.stream_path(rep)
            else:
    ...
    def _get_version(self) -> list:
        ...
        @TODO: add documentation
        """
        version = "7" if self.media.options.get(
            'hls_segment_type', '') == 'fmp4' else "3"
